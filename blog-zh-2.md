# TouchResume – AI 智能简历生成器，50 套模板、拖拽编辑、职业照生成、多格式导出

## TouchResume 是什么

> 官网：https://touchresume.cturing.cn/zh | GitHub：https://github.com/twwch/TouchResume

TouchResume 是一款开源的 AI 智能简历生成器，基于 AI 技术帮助用户快速创建高质量的专业简历。工具完全免费，支持自托管部署，Docker 一行命令即可启动。内置 50 套专业设计模板，提供拖拽式所见即所得编辑器，集成 AI 对话优化、一键生成简历、语法检查、JD 匹配分析、职业照生成等丰富的 AI 能力。所有 AI 功能使用用户自己的 API Key，密钥仅存储在浏览器本地，数据安全有保障。TouchResume 支持 PDF、DOCX、HTML 等多格式导出，还可以生成分享链接直接发给 HR，让简历制作变得简单而智能。
...
- **在线使用**：直接访问 [touchresume.cturing.cn/zh](https://touchresume.cturing.cn/zh)，无需注册即可开始使用。
- **自托管部署**：通过 Docker 一行命令部署（`docker run -d -p 3000:3000 -e AUTH_SECRET=<密钥> -v touchresume-data:/app/data twwch/touchresume:latest`），也可以本地 clone 源码开发。
...
- **开源协议**：Apache 2.0
- **官网**：https://touchresume.cturing.cn/zh
