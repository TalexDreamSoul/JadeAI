...
const HIGHLIGHT = 'var(--resume-accent-color,#fbbf24)';

function buildArtisticSectionContent(section: Section, lang: string): string {
  const c = section.content as any;

  if (section.type === 'summary') {
    return `<div class="rounded-lg p-4" style="border:2px dashed var(--resume-accent-color-30);background-color:var(--resume-accent-color-08)"><div class="text-sm leading-relaxed text-zinc-600 italic">${md((c as SummaryContent).text)}</div></div>`;
  }

  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any) => `<div class="relative rounded-lg p-4" style="border:1px dashed var(--resume-accent-color-30)">
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px dashed var(--resume-accent-color-30)">
  ...
  if (section.type === 'skills') {
    return `<div class="space-y-3">${((c as SkillsContent).categories || []).map((cat: any) => `<div>
      ...
      <div class="flex flex-wrap gap-2">${(cat.skills || []).map((skill: string, i: number) =>
        `<span class="flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium" style="background-color:var(--resume-accent-color-10);color:${PRIMARY}"><span class="h-2 w-2 rounded-full" style="background-color:${i % 2 === 0 ? ACCENT : HIGHLIGHT}"></span>${esc(skill)}</span>`
      ).join('')}</div>
    </div>`).join('')}</div>`;
  }

  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px dashed var(--resume-accent-color-30)">
  ...
  if (section.type === 'certifications') {
    return `<div class="flex flex-wrap gap-2">${((c as CertificationsContent).items || []).map((it: any) =>
      `<div class="rounded-lg px-4 py-2" style="border:1px dashed var(--resume-accent-color-30);background-color:var(--resume-accent-color-10)"><p class="text-sm font-bold" style="color:${PRIMARY}">${esc(it.name)}</p>${it.issuer || it.date ? `<p class="text-xs text-zinc-500">${it.issuer ? esc(it.issuer) : ''}${it.issuer && it.date ? ' | ' : ''}${it.date ? esc(it.date) : ''}</p>` : ''}</div>`
    ).join('')}</div>`;
  }

  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-3">${((c as LanguagesContent).items || []).map((it: any) =>
      `<div class="flex items-center gap-2 rounded-full px-4 py-1.5" style="border:2px dashed var(--resume-accent-color-30)"><span class="h-2.5 w-2.5 rounded-full" style="background-color:${ACCENT}"></span><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.language)}</span><span class="text-xs text-zinc-400">${esc(it.proficiency)}</span></div>`
    ).join('')}</div>`;
  }

  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px dashed var(--resume-accent-color-30)">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px dashed var(--resume-accent-color-30)">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-lg p-3" style="border:1px dashed var(--resume-accent-color-30)"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm text-zinc-600">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
  ...
  return `<div class="mx-auto max-w-[210mm] overflow-hidden bg-white shadow-lg" style="font-family:Inter,sans-serif">
    ...
    <div class="p-8 pt-6">
      ${sections.map(s => `<div class="mb-6" data-section data-section-type="${esc(s.type)}">
        <div class="mb-2 flex items-center gap-2"><h2 class="text-sm font-extrabold uppercase tracking-widest" style="color:${PRIMARY}">${esc(s.title)}</h2><div class="h-0.5 flex-1" style="border-top:2px dashed var(--resume-accent-color-40)"></div></div>
