...
export function buildTouchLineHtml(resume: ResumeWithSections): string {
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Inter,Arial,sans-serif;color:#111827">
    <header class="mb-7 grid grid-cols-[1fr_auto] items-start gap-6 border-b-2 border-zinc-950 pb-5"><div><p class="mb-2 text-xs font-semibold uppercase tracking-[0.35em] text-zinc-400">Resume</p><h1 class="text-4xl font-black tracking-[-0.04em] text-zinc-950">${esc(pi.fullName || 'Your Name')}</h1>${contactItems ? `<p class="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-sm text-zinc-500">${contactItems}</p>` : ''}</div>${pi.avatar ? `<img src="${esc(pi.avatar)}" alt="" class="shrink-0 border border-zinc-200 object-cover" style="width:62px;height:86px;border-radius:4px"/>` : ''}</header>
