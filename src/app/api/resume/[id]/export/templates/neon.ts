...
function buildNeonSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'summary') {
    return `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)"><div class="text-sm leading-relaxed" style="color:${TEXT}">${md((c as SummaryContent).text)}</div></div>`;
  }

  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)">
      <div class="flex items-baseline justify-between"><h3 class="text-sm font-bold" style="color:${CYAN}">${esc(it.position)}</h3><span class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold" style="color:${BG};background-color:${VIOLET};box-shadow:0 0 8px var(--resume-accent-color-40)">${esc(it.startDate)} - ${esc(it.endDate) || (it.current ? (lang === 'zh' ? '至今' : 'Present') : '')}</span></div>
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)">
  ...
  if (section.type === 'certifications') {
    return `<div class="flex flex-wrap gap-2">${((c as CertificationsContent).items || []).map((it: any) =>
      `<div class="rounded-lg px-4 py-2" style="border:1px solid var(--resume-accent-color-30);background-color:var(--resume-accent-color-08)"><p class="text-sm font-bold" style="color:${CYAN}">${esc(it.name)}</p>${it.issuer || it.date ? `<p class="text-xs" style="color:${TEXT_DIM}">${it.issuer ? esc(it.issuer) : ''}${it.issuer && it.date ? ' | ' : ''}${it.date ? esc(it.date) : ''}</p>` : ''}</div>`
    ).join('')}</div>`;
  }

  if (section.type === 'languages') {
    return `<div class="flex flex-wrap gap-3">${((c as LanguagesContent).items || []).map((it: any) =>
      `<div class="flex items-center gap-2 rounded-full px-4 py-1.5" style="border:1px solid var(--resume-accent-color-30)"><span class="h-2 w-2 rounded-full" style="background-color:${CYAN};box-shadow:0 0 6px ${CYAN}"></span><span class="text-sm font-medium" style="color:${CYAN}">${esc(it.language)}</span><span class="text-xs" style="color:${TEXT_DIM}">${esc(it.proficiency)}</span></div>`
    ).join('')}</div>`;
  }

  if (section.type === 'github') {
    return `<div class="space-y-3">${((c as GitHubContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)">
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it: any) => `<div class="rounded-lg p-4" style="border:1px solid var(--resume-accent-color-20);background-color:var(--resume-accent-color-08)">
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-lg p-3" style="border:1px solid var(--resume-accent-color-20)"><span class="text-sm font-medium" style="color:${CYAN}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<div class="text-sm" style="color:${TEXT}">${md(it.description)}</div>` : ''}</div>`).join('')}</div>`;
  }
...
export function buildNeonHtml(resume: ResumeWithSections): string {
  ...
  return `<div class="mx-auto max-w-[210mm] overflow-hidden shadow-lg" style="font-family:Inter,sans-serif;background-color:${BG}">
    <div class="relative px-10 py-8" style="border-bottom:2px solid ${CYAN};box-shadow:0 2px 20px var(--resume-accent-color-40)">
      <div class="flex items-center gap-5">
        ${pi.avatar ? `<div class="shrink-0 rounded-lg p-0.5" style="border:2px solid ${CYAN};box-shadow:0 0 12px var(--resume-accent-color-50)"><img src="${esc(pi.avatar)}" alt="" class="h-20 w-20 rounded-lg object-cover"/></div>` : ''}
        <div>
          <h1 class="text-3xl font-extrabold tracking-tight" style="color:${CYAN};text-shadow:0 0 20px var(--resume-accent-color-50)">${esc(pi.fullName || 'Your Name')}</h1>
          ${pi.jobTitle ? `<p class="mt-1 text-sm font-medium" style="color:${VIOLET};text-shadow:0 0 10px var(--resume-accent-color-40)">${esc(pi.jobTitle)}</p>` : ''}
          ${contacts.length ? `<div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs" style="color:${TEXT_DIM}">${contacts.map((c, i) => `<span class="flex items-center gap-1.5">${esc(c)}${i < contacts.length - 1 ? `<span style="color:var(--resume-accent-color-40)">|</span>` : ''}</span>`).join('')}</div>` : ''}
        </div>
      </div>
    </div>
    <div class="p-8 pt-6">
      ${sections.map(s => `<div class="mb-6" data-section data-section-type="${esc(s.type)}">
        <div class="mb-3 flex items-center gap-3"><h2 class="text-sm font-extrabold uppercase tracking-widest" style="color:${CYAN};text-shadow:0 0 10px var(--resume-accent-color-40)">${esc(s.title)}</h2><div class="h-px flex-1" style="background:linear-gradient(90deg,var(--resume-accent-color-40),transparent)"></div></div>
