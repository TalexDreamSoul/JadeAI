...
function flatItem(title: string, subtitle = '', date = '', description = '', technologies?: string[], highlights?: string[]) {
  return `<div class="rounded-xl bg-zinc-50 px-3 py-2">
    <div class="flex items-baseline justify-between gap-4"><div class="min-w-0"><span class="text-sm font-semibold text-zinc-900">${esc(title)}</span>${subtitle ? `<span class="text-sm text-zinc-600"> · ${esc(subtitle)}</span>` : ''}</div>${date ? `<span class="shrink-0 text-xs text-zinc-500">${esc(date)}</span>` : ''}</div>
...
function buildSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'work_experience') return `<div class="space-y-3">${((c as WorkExperienceContent).items || []).map((it: any) => flatItem(it.position, [it.company, it.location].filter(Boolean).join(' · '), dateRange(it.startDate, it.endDate, it.current, lang), it.description, it.technologies, it.highlights)).join('')}</div>`;
  if (section.type === 'education') return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => flatItem(degreeField(it.degree, it.field), [it.institution, it.location].filter(Boolean).join(' · '), dateRange(it.startDate, it.endDate, false, lang), it.gpa ? `GPA: ${it.gpa}` : '', undefined, it.highlights)).join('')}</div>`;
  if (section.type === 'skills') return `<div class="grid gap-2 sm:grid-cols-2">${((c as SkillsContent).categories || []).map((cat: any) => `<div class="rounded-xl bg-zinc-50 px-3 py-2"><div class="mb-1 text-xs font-semibold text-zinc-500">${esc(cat.name)}</div><div class="flex flex-wrap gap-1.5">${(cat.skills || []).map((skill: string) => `<span class="rounded-md bg-white px-2 py-0.5 text-xs text-zinc-700 ring-1 ring-zinc-200">${esc(skill)}</span>`).join('')}</div></div>`).join('')}</div>`;
  if (section.type === 'projects') return `<div class="space-y-3">${((c as ProjectsContent).items || []).map((it: any) => flatItem(it.name, '', dateRange(it.startDate, it.endDate, false, lang), it.description, it.technologies, it.highlights)).join('')}</div>`;
  ...
  if (section.type === 'github') return `<div class="space-y-2">${((c as GitHubContent).items || []).map((it: any) => flatItem(it.name, '', `★ ${it.stars?.toLocaleString() ?? 0}`, it.description)).join('')}</div>`;
  if (section.type === 'custom') return `<div class="space-y-2">${((c as CustomContent).items || []).map((it: any) => flatItem(it.title, it.subtitle, it.date, it.description)).join('')}</div>`;
