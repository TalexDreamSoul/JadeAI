...
export function buildTouchFlatHtml(resume: ResumeWithSections): string {
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Inter,Arial,sans-serif;color:#111827">
    <header class="mb-5 rounded-2xl bg-zinc-50 px-5 py-4 ring-1 ring-zinc-100"><div class="flex items-center justify-between gap-5"><div class="min-w-0 flex-1"><h1 class="text-3xl font-bold tracking-tight text-zinc-950">${esc(pi.fullName || 'Your Name')}</h1>${contactItems ? `<p class="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm text-zinc-500">${contactItems}</p>` : ''}</div>${pi.avatar ? `<img src="${esc(pi.avatar)}" alt="" class="shrink-0 object-cover" style="width:58px;height:81px;border-radius:4px"/>` : ''}</div></header>
