...
function buildModernSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:var(--resume-accent-color,#e94560)">
      ...
      ${it.company ? `<p class="text-sm" style="color:var(--resume-accent-color,#e94560)">${esc(it.company)}</p>` : ''}
  ...
  if (section.type === 'education') {
    return `<div class="space-y-3">${((c as EducationContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:var(--resume-primary-color,#0f3460)">
  ...
  if (section.type === 'projects') {
    return `<div class="space-y-4">${((c as ProjectsContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:var(--resume-accent-color,#e94560)">
  ...
  if (section.type === 'github') {
    return `<div class="space-y-4">${((c as GitHubContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:var(--resume-accent-color,#e94560)">
  ...
  if (section.type === 'certifications') {
    return `<div class="space-y-1.5">${((c as CertificationsContent).items || []).map((it: any) =>
      `<div class="flex items-baseline justify-between border-l-2 pl-4" style="border-color:var(--resume-primary-color,#0f3460)"><div><span class="text-sm font-semibold text-zinc-800">${esc(it.name)}</span>${it.issuer ? `<span class="text-sm text-zinc-500"> — ${esc(it.issuer)}</span>` : ''}</div>${it.date ? `<span class="shrink-0 text-xs text-zinc-400">${esc(it.date)}</span>` : ''}</div>`
    ).join('')}</div>`;
  }
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it: any) => `<div class="border-l-2 pl-4" style="border-color:var(--resume-accent-color,#e94560)">
...
export function buildModernHtml(resume: ResumeWithSections): string {
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Inter,sans-serif">
    <div class="relative overflow-hidden px-10 py-8 text-white" style="background:linear-gradient(135deg,var(--resume-primary-color,#1a1a2e) 0%,var(--resume-accent-color,#16213e) 50%,var(--resume-primary-color,#0f3460) 100%)">
      <div class="absolute -right-10 -top-10 h-40 w-40 rounded-full opacity-10" style="background:radial-gradient(circle,var(--resume-accent-color,#e94560) 0%,transparent 70%)"></div>
      <div class="absolute -bottom-6 right-20 h-24 w-24 rounded-full" style="opacity:0.08;background:radial-gradient(circle,var(--resume-accent-color,#e94560) 0%,transparent 70%)"></div>
      <div class="relative flex items-center gap-6">
        ${pi.avatar ? `<div class="shrink-0 rounded-full p-[2px]" style="background:linear-gradient(135deg,var(--resume-accent-color,#e94560),var(--resume-primary-color,#0f3460))"><img src="${esc(pi.avatar)}" alt="" class="h-[80px] w-[80px] rounded-full border-2 border-white/10 object-cover"/></div>` : ''}
        <div class="min-w-0 flex-1">
          <h1 class="text-3xl font-bold tracking-tight">${esc(pi.fullName || 'Your Name')}</h1>
          ${pi.jobTitle ? `<p class="mt-1.5 text-base font-light tracking-wide" style="color:var(--resume-accent-color,#e94560)">${esc(pi.jobTitle)}</p>` : ''}
      ...
      <div class="absolute bottom-0 left-0 h-[3px] w-full" style="background:linear-gradient(90deg,var(--resume-accent-color,#e94560) 0%,var(--resume-primary-color,#0f3460) 60%,transparent 100%)"></div>
    </div>
    <div class="p-8 pt-6">
      ${sections.map(s => `<div class="mb-6" data-section data-section-type="${esc(s.type)}">
        <h2 class="mb-3 flex items-center gap-2.5 text-sm font-bold uppercase tracking-wider" style="color:var(--resume-accent-color,#e94560)">
          <span class="h-[3px] w-7 rounded-full" style="background:linear-gradient(90deg,var(--resume-accent-color,#e94560),var(--resume-primary-color,#0f3460))"></span>${esc(s.title)}
        </h2>
