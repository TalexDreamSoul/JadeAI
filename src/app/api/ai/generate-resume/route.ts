...
Respond with JSON only.`;
    ...
    const generated = await withMeteredAIUsage({
      ...
      run: async () => {
        const generate = (structuredOutput: boolean) => generateText({
          model,
          // A full six-section resume can exceed 8192 output tokens.
          maxOutputTokens: 16384,
          system: getSystemPrompt(lang),
          prompt: promptText,
          providerOptions: getProviderOptions(aiConfig),
          output: structuredOutput ? Output.json() : undefined,
        });
        ...
        try {
          ...
          // Some OpenAI-compatible endpoints reject structured JSON mode.
          if (aiConfig.provider !== 'openai') throw error;
