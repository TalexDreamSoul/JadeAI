...
export const interviewRepository = {
  ...
  async countSessionsForUserSince(userId: string, since: Date) {
    const rows = await db
      .select({ id: interviewSessions.id })
      .from(interviewSessions)
      .where(and(eq(interviewSessions.userId, userId), gte(interviewSessions.createdAt, since)));
    return rows.length;
  },
  ...
  async setRoundSummary(roundId: string, summary: RoundSummary) {
    await db.update(interviewRounds).set({ summary, updatedAt: new Date() }).where(eq(interviewRounds.id, roundId));
  },
  ...
  async updateMessageMetadata(messageId: string, metadata: InterviewMessageMetadata) {
    await db.update(interviewMessages).set({ metadata }).where(eq(interviewMessages.id, messageId));
  },
